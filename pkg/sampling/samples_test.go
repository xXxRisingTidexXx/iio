...
var _ = Describe("samples", func() {
	Context("creation", func() {
		It("should construct from nil slice", func() {
			defer ExpectNoPanic()
			Expect(sampling.New().Length()).To(Equal(0))
		})
		It("should construct from zero-length slice", func() {
			defer ExpectNoPanic()
			Expect(sampling.New(make([]*sampling.Sample, 0)...).Length()).To(Equal(0))
		})
	...
	Context("comparison", func() {
		...
		It("should equate samples from nil & non-nil slices", func() {
			defer ExpectNoPanic()
			Expect(cmp.Equal(sampling.New(), sampling.New(make([]*sampling.Sample, 0)...))).To(BeTrue())
		})
